...
if select == 3:
    ...
    #damping = 0.1 # step factor (called alpha in the lecture slides) - example without convergence
    damping = 0.005 # step factor (called alpha in the lecture slides)

    pg = K(q)    
    diff = pt - pg
    while diff.length() > 0.5: # threshold of how close we want to get to the target
        rfig.clear()
...
if select == 4:
    ...
    while diff.length() > 0.1: # threshold of how close we want to get to the target
        # NOTE: this treshold is much smaller than for Gradient Descent!!!
